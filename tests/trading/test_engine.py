...
class TestFetchLivePositions:
    def test_returns_positions(self, engine, mock_collector):
        mock_collector.fetch_positions = MagicMock(
            return_value=[
                {
...
class TestCloseAllLongPositions:
    def test_closes_long_positions(self, engine, mock_collector):
        mock_collector.fetch_account_info = MagicMock(
            return_value=MagicMock(available_balance=1000.0, account_status="NORMAL")
        )
        mock_collector.fetch_positions = MagicMock(
            return_value=[
                {
        ...
        mock_collector.close_position = MagicMock(
    ...
    def test_risk_check_failure_returns_empty(self, engine, mock_collector):
        mock_collector.fetch_account_info = MagicMock(
            return_value=MagicMock(available_balance=1.0, account_status="NORMAL")
        )
